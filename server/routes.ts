...
// AI Configuration - Using mock for demo
const USE_MOCK_AI = true; // Set to false when you have working API
...
async function callHuggingFaceAPI(prompt: string): Promise<string> {
  ...
  if (USE_MOCK_AI) {
    console.log("Using mock AI response for client demo");
  ...
  try {
    ...
    console.error("Hugging Face API error, using mock response");
    return await generateMockAIResponse(prompt);
  }
}

function generateMockAIResponse(prompt: string): Promise<string> {
  return new Promise((resolve) => {
    // Extract the goal from the prompt
    let goal = "your project";
    if (prompt.includes("Goal:")) {
      const goalMatch = prompt.match(/Goal: (.+?)(?:\n|$)/);
      goal = goalMatch ? goalMatch[1] : "your project";
    }

    // Simulate API delay
    setTimeout(() => {
      const mockResponse = {
        projectTitle: `Project: ${goal.substring(0, 30)}${goal.length > 30 ? '...' : ''}`,
        projectDescription: `A comprehensive plan to successfully complete: ${goal}`,
        tasks: [
          {
            title: "Research and Requirements Gathering",
            description: "Conduct thorough research and define clear project requirements",
          ...
          },
          {
            ...
            description: "Set up development environment and create detailed project timeline",
            priority: "high" as const,
            estimatedHours: 4,
            tags: ["setup", "planning"]
          },
          ...
          },
          {
            ...
            description: "Test all components thoroughly and ensure quality standards",
            priority: "medium" as const,
            estimatedHours: 8,
            tags: ["testing", "qa"]
          },
          {
            ...
            description: "Create comprehensive documentation and user instructions",
          ...
          },
          {
            ...
            description: "Conduct final review and deploy the completed project",
            priority: "high" as const,
            estimatedHours: 3,
  ...
  try {
    ...
    return {
      ...
      tasks: lines.slice(3).map((line, idx) => ({
        title: line.replace(/^[-•*]\s*/, '') || `Task ${idx + 1}`,
    ...
    // Return a sensible default with guaranteed tasks array
    return {
  ...
  app.post("/api/ai/create-project", async (req, res) => {
    try {
      ...
      const project = await storage.createProject({
        title: breakdown.projectTitle || "AI Generated Project",
        description: breakdown.projectDescription || "Created from AI breakdown",
        status: 'active',
        priority: 'medium',
        progress: 0,
      });
